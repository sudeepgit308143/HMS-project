...
public class AdminController {
	...
	public String  editDoctor(Model  model,@PathVariable("id") int id) {
		DoctorBean doctBean = doctorService.getDoctorById(id);
		model.addAttribute("doctBean", doctBean);
		return "adminEditBoard";
	}
	
	@PostMapping(value="/doctorMgmt/updateDoctor")
	public String updatePatient(@ModelAttribute("doctBean") DoctorBean doctBean,Model model) {
		System.out.println("add Patient ");
		System.out.println(doctBean.getId());
		System.out.println(doctBean.getEmail());
		doctBean = doctorService.updateDoctor(doctBean);
		if(doctBean != null && doctBean.getId() >0) {
			List<DoctorBean> docBeanList = doctorService.getAllDoctors();
			model.addAttribute("docBeanList", docBeanList);
		}
	...
	@GetMapping(value="/doctorMgmt/deleteDoctor/{id}")
